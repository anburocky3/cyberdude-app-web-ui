...
const dnaItems = [
  { icon: Zap, name: "Grit & Persistence", desc: "Measures the ability to persevere through difficult challenges without giving up." },
  { icon: Users, name: "Collaboration Index", desc: "Tracks how effectively a learner works with others and contributes to the community." },
  { icon: TrendingUp, name: "Learning Velocity", desc: "Measures how quickly a learner acquires and masters new, complex concepts." },
  { icon: MessageSquare, name: "Communication Clarity", desc: "Assesses the ability to articulate questions and solutions clearly to teammates and clients." },
  { icon: Clock, name: "Consistency Quotient", desc: "Rewards steady, reliable effort over time rather than sporadic bursts of activity." },
  { icon: Brain, name: "Problem-Solving Methodology", desc: "Analyzes how a learner solves problems — methodical and data-driven, or creative brainstormer." },
];
...
const flowSteps = [
  ...
  { label: "Verified Competence", sub: "Evidence-backed skills", highlight: false },
...
function ProblemCard({ item, index }: { item: typeof problems[0]; index: number }) {
  ...
  const background = useTransform(
    [x, y],
    ([latestX, latestY]) => `radial-gradient(400px circle at ${latestX}px ${latestY}px, color-mix(in srgb, var(--color-orange-500) 120%, transparent), transparent 50%)`
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 p-8 flex flex-col h-full">
        ...
        <div className="text-3xl font-black text-orange-500 tabular-nums mb-3 leading-none drop-shadow-sm">{item.stat}</div>
        <h3 className="text-xl font-black text-foreground mb-3 leading-snug">{item.title}</h3>
        <p className="text-sm text-muted-foreground leading-relaxed mt-auto pr-4">{item.desc}</p>
      </div>
...
function TimelineNode({ item, index, isEven }: { item: typeof dnaItems[0]; index: number; isEven: boolean }) {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: "-20%" });

  return (
    <div ref={ref} className="relative flex items-center mb-12 last:mb-0 w-full group">
      {/* Node pin */}
      ...
      <motion.div
        ...
        className={`absolute hidden md:block top-1/2 h-px bg-border/40 group-hover:bg-orange-500/50 transition-colors z-0 ${isEven ? "right-1/2 origin-right w-16" : "left-1/2 origin-left w-16"
          }`}
      />
      ...
      >
        <div className="p-7 md:p-8 rounded-3xl border border-border/30 bg-card/5 hover:bg-card/30 hover:shadow-lg hover:border-orange-500/40 transition-all duration-300 relative overflow-hidden">
          ...
          <div className="flex items-center gap-5 mb-5 relative z-10">
            ...
            <div>
              <span className="text-xs font-black tracking-widest uppercase text-orange-500/60 block mb-1">Dimension 0{index + 1}</span>
              <h3 className="text-xl font-black text-foreground">{item.name}</h3>
            </div>
          </div>
          <p className="text-muted-foreground leading-relaxed text-sm relative z-10">{item.desc}</p>
        </div>
      </motion.div>
    </div>
  );
...
function PremiumStepper({ steps }: { steps: typeof flowSteps }) {
  const ref = useRef(null);
  const { scrollYProgress } = useScroll({ target: ref, offset: ["start 85%", "end center"] });
  const inView = useInView(ref, { once: true, margin: "-10%" });

  return (
    <div ref={ref} className="relative py-8 md:py-12 px-2 overflow-x-hidden md:overflow-x-visible">
        ...
        {steps.map((step, i) => (
          ...
          >
            ...
            <motion.div
              ...
              className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl shrink-0 rotate-45 border-2 flex items-center justify-center mr-6 md:mr-0 md:mb-8 bg-card transition-all duration-300 relative z-10 ${step.highlight
            ...
            >
              <div className={`-rotate-45 font-black text-lg md:text-xl tabular-nums ${step.highlight ? 'text-orange-500' : 'text-muted-foreground group-hover:text-foreground transition-colors'}`}>
                0{i + 1}
              </div>
            </motion.div>

            <div className="text-left md:text-center w-full max-w-xs md:max-w-none">
              <h4 className={`text-base md:text-lg font-black mb-1.5 leading-tight ${step.highlight ? 'text-orange-500 drop-shadow-sm' : 'text-foreground'}`}>
                {step.label}
              </h4>
...
/* ── Hover Glow Card ────────────────────────────────────── */
function HoverGlowCard({ children, className, innerClassName }: { children: React.ReactNode; className?: string; innerClassName?: string }) {
  return (
    <motion.div 
      initial="initial"
      whileHover="hover"
      variants={{
        initial: { scale: 1, y: 0 },
        hover: { scale: 1.01, y: -4, transition: { duration: 0.4, ease: "easeOut" } }
      }}
    ...
    >
      ...
      <div className="absolute inset-0 z-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 overflow-hidden rounded-3xl pointer-events-none">
        ...
        <motion.div
           ...
           style={{
             backgroundImage: "conic-gradient(from 0deg, transparent 0 250deg, color-mix(in srgb, var(--color-orange-500) 70%, transparent) 320deg, var(--color-orange-500) 360deg)"
           }}
        />
        ...
        <motion.div
           ...
           style={{
             backgroundImage: "conic-gradient(from 0deg, transparent 0 280deg, color-mix(in srgb, var(--color-orange-500) 80%, transparent) 330deg, var(--color-orange-500) 360deg)"
           }}
        />
      </div>
      ...
      {innerClassName && (
        <div className={`absolute inset-0.5 z-0 rounded-2xl pointer-events-none ${innerClassName}`} />
      )}
    ...
    <div className="bg-background text-foreground antialiased overflow-x-hidden">
          ...
          <div className="grid lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            <HoverGlowCard>
              <div className="relative flex flex-col justify-between p-10 md:p-14 h-full overflow-hidden">
                ...
                <motion.div
                  variants={{
                    ...
                    hover:   { opacity: 0.04, scale: 1.5, rotate: -15, x: "10%", y: "-10%", transition: { duration: 0.7, ease: "easeOut" } }
                  }}
                ...
                >
                  To bridge the{" "}
                  <strong className="text-foreground">&quot;Theory vs. Practice&quot;</strong> divide in digital learning by
                  creating an accountable, evidence-based environment where completion actually means competence — through
                  Natural Intelligence, strict monitoring, and continuous engagement.
                </motion.p>
              </div>
              ...
              <motion.div
                ...
                variants={{
                  hover: { x: 10, backgroundColor: "color-mix(in srgb, var(--color-orange-500) 10%, transparent)", borderColor: "color-mix(in srgb, var(--color-orange-500) 40%, transparent)" }
                }}
              ...
              >
                <motion.div variants={{ hover: { rotate: 15, scale: 1.2 } }} transition={{ duration: 0.3 }}>
                  <Zap size={14} className="text-orange-500 shrink-0" />
                </motion.div>
                <span className="text-sm font-bold text-foreground">We engineer competence.</span>
              </motion.div>
              </div>
            </HoverGlowCard>
            ...
            <HoverGlowCard innerClassName="bg-secondary/5">
              <div className="relative flex flex-col justify-between p-10 md:p-14 h-full overflow-hidden">
                ...
                <div className="absolute bottom-6 right-6 text-8xl font-black text-foreground/5 leading-none select-none tracking-tighter">
                </div>
              ...
              <div className="relative z-10">
                  ...
                  {["The global", "benchmark."].map((line, i) => (
                    <div key={i} className="overflow-hidden">
                      <motion.div
                        initial={{ y: "110%" }}
                        animate={{ y: 0 }}
                        transition={{ duration: 0.7, delay: 0.22 + i * 0.1, ease }}
                      >
                        {line}
                      </motion.div>
                    </div>
                ...
                <motion.p
                  ...
                  className="text-base text-muted-foreground leading-relaxed max-w-sm"
                >
                  To be the global benchmark for &quot;Natural Intelligence&quot; in education — where every learner&apos;s journey is
                  defined by verified grit, logic, and practical mastery rather than passive participation. A future where
                  recruiters, institutions, and guardians have{" "}
                  <strong className="text-foreground">total transparency and trust</strong> in digital credentials.
                </motion.p>
              </div>
              ...
              <motion.div
                ...
                variants={{
                  hover: { x: 10, backgroundColor: "color-mix(in srgb, var(--color-orange-500) 150%, transparent)", borderColor: "color-mix(in srgb, var(--color-orange-500) 50%, transparent)" }
                }}
              ...
              >
                ...
                <span className="text-sm font-bold text-foreground">
                  Powered by <span className="text-orange-500">Grit, Logic &amp; Consistency</span>
                </span>
              </motion.div>
              </div>
            </HoverGlowCard>
          ...
          <div className="grid lg:grid-cols-[1fr_1.6fr] gap-10 mb-16 items-end">
            ...
            <FadeIn from="right" delay={0.15}>
              <p className="text-base text-muted-foreground leading-relaxed">
                Traditional online learning is plagued by low completion rates and a lack of trust from employers.
                CyberDude.app was engineered to fix these specific, measurable pain points.
              </p>
            </FadeIn>
          </div>
      ...
      <section className="bg-orange-500 relative overflow-hidden py-16 px-8 md:px-14">
        ...
        <div className="max-w-4xl mx-auto relative z-10 text-center">
          ...
          >
            A professional is more than a list of technical skills. Skill DNA tracks behavioral and cognitive scores
            that show recruiters the <strong className="text-orange-50">true potential</strong> of a candidate.
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-24 px-8 md:px-14 bg-secondary/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-[1fr_2fr] gap-12 mb-14 items-end">
            ...
            <FadeIn from="right" delay={0.15}>
              <p className="text-base text-muted-foreground leading-relaxed">
                Every interaction on the platform contributes to a learner&apos;s Skill DNA profile — a live, verified
                record of how they actually think, work, and collaborate.
              </p>
            </FadeIn>
          </div>
              ...
              <div className="max-w-xl">
                ...
                <FadeIn delay={0.2}>
                  <p className="text-muted-foreground text-base mt-4 leading-relaxed">
                    Stop collecting certificates. Start building real, verifiable skills that recruiters can trust —
                    backed by Skill DNA, not just a completion badge.
                  </p>
                </FadeIn>
              </div>
